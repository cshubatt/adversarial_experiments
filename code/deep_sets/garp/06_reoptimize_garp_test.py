...
odir = 'data_k{0}_nsim{1}/outfiles/'.format(k, nsims)
f1 = open(odir + f'06_reoptimize_garp_test_stdout_{num_points}.log', 'w')
f2 = open(odir + f'06_reoptimize_garp_test_stderr_{num_points}.log', 'w')
sys.stdout = f1
sys.stderr = f2

# Load the trained model
model_path = 'data_k{0}_nsim{1}/optimized_points_{2}/finetune_best_mse_model.pth'.format(k, nsims, num_points)
...
bounds = [(0, 100) for _ in range(num_points * 2)]
...
predictions = []
odir = 'data_k{0}_nsim{1}/optimized_points_{2}/finetune/'.format(k, nsims, num_points)
...
for i in range(20000):
    ...
    # Initial guess for the points (num_points points in [0, 100]^2)
    initial_guess = np.random.uniform(0, 100, size=(num_points, 2)).flatten()
